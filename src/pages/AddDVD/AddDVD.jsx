...
function AddDVD() {
      ...
      const addImageHandler=(type)=>{
            ...
            if(type === 'less'){
                  numberOFDvdImage = (numberOFDvdImage>1)? numberOFDvdImage-1 : numberOFDvdImage;

                  delete inputNewStatus['image'][numberOFDvdImage];

                  (numberOFDvdImage>1)&&(
                        dvdData.dvDimages.pop()
                  )
                  ...
                  dvdData.dvDimages.push({...dvdImg});
            }
            
            setNumberOfDvdImage(numberOFDvdImage);
            setDvdData({...dvdData})
      }
                  ...
                  <section>
                        ...
                        <form id="add-dvd-form" className="row g-3" onSubmit={submitButtonHandler}  onReset={resetButtonHandler}>
                              ...
                              <div className="col-md-6">
                                    ...
                                    <input type="number" className="form-control input--design" id="inputPrice" name='standardCharge' value={dvdData.standardCharge} onChange={inputChangeHandler} min={1}  pattern="[1-9]+" required/>
                              </div>
                              {/* penalty */}
                              <div className="col-md-6">
                                    <label htmlFor="inputPenalty" className="form-label">Penalty</label>
                                    <input type="number" className="form-control input--design" id="inputPenalty" name='penaltyCharge' value={dvdData.penaltyCharge} onChange={inputChangeHandler} min={0} max={dvdData.standardCharge-1} pattern="[0-9]+" required/>
                              </div>
                                          ...
                                          {(!inputNewStatus.producer)
                                                ?(    
                                                      ...
                                                      <select className="form-select input--design" name='producerNumber' id="dvdProducer" value={dvdData.dvdProducer.producerNumber} onChange={inputChangeHandler} required>
                                                            <option value={''}>---</option>
                                                            {nameList.producers.map(({producerNumber, producerName}, index)=>{return(
                                          ...
                                          {(!inputNewStatus.studio)
                                                ?(    
                                                      ...
                                                      <select className="form-select input--design" name='studioNumber' id="dvdStudio" value={dvdData.dvdStudio.studioNumber} onChange={inputChangeHandler} required>
                                                            <option value={''}>---</option>
                                                            {nameList.studios.map(({studioNumber, studioName}, index)=>{return(
                              ...
                              <section className="col-md-6" >
                                    ...
                                    {[...Array(numberOFActors)].map((a, index)=>{return(
                                                ...
                                                {(!inputNewStatus.actor[index])
                                                      ?(
                                                            ...
                                                            <select className="form-select input--design" name='actorNumber' id="actors" value={dvdData.actors[index].actorNumber} onChange={(e)=>inputChangeHandler(e, index)} required>
                                                                  <option value={''}>---</option>
                                                                  {nameList.actors.map(({actorId, actorName}, index)=>{return(
                                          ...
                                          {(!inputNewStatus.cateogary)
                                                ?(    
                                                      ...
                                                      <select className="form-select input--design"  name='categoryNumber' id="dvdCategory" value={dvdData.dvdCategory.categoryNumber} onChange={inputChangeHandler} required>
                                                            <option value={''}>---</option>
                                                            {nameList.cateogary.map(({categoryNumber, categoryDescription}, index)=>{return(
                              ...
                              <section className="col-md-6" >
                                    <label htmlFor="inputImage" className="form-label">
                                          Image 
                                          <a style={{color: 'blue', paddingBottom:'0rem', paddingLeft:'0.2rem', cursor:'pointer', fontSize:'0.9rem'}} onClick={()=>addImageHandler('more')}> add more</a>
                                          <a style={{color: 'blue', paddingBottom:'0rem', paddingLeft:'0.2rem', cursor:'pointer', fontSize:'0.9rem'}} onClick={()=>addImageHandler('less')}> add Less</a>
                                    </label>
                                    
                                    {/* loop to add muliple image */}
                                    {[...Array(numberOFDvdImage)].map((a, index)=>{return(
                                          <div className={`d-flex gap-3 ${(index>0)&&'mt-2'}`} key={`imageInput${index}`}>
                                                {(!inputNewStatus.image[index])
                                                      ?(
                                                            <>
                                                            <input type="file" accept=".gif,.jpg,.jpeg,.png,.doc,.docx" className="form-control input--design" name='image64' id="dvDimages" placeholder='insert Name....' value={dvdData.dvDimages[index].image64} onChange={(e)=>inputChangeHandler(e, index)}/>
                                                            </>
                                                      )
                                                      :(
                                                            <>
